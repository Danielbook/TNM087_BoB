...
fpts = ginput(3);
% First two is x axis, third y axis

%% Generate the elliptical mask and
% set all points in MImage outside the mask to black
centerDist = sqrt( (fpts(1,1)-fpts(2,1)).^2 + (fpts(1,2)-fpts(2,2)).^2 );
centerP = [ fpts(2,1)-(fpts(1,1)), fpts(2,2) ];
a = centerDist/2;
b = sqrt( (fpts(3,1)-centerP(1,1)).^2 + (fpts(3,2)-centerP(1,2)).^2 );

fmask = ((R-centerDist).^2./a^2)+((C-centerDist).^2./b^2)<=1;% this is the mask use C and R to generate it
fmask = uint8(fmask);
...
emask = (R - epts(1)).^2 + (C - epts(1)).^2 < 1;% circular eye mask again, use C and R
emask =+ (R - epts(2)).^2 + (C - epts(2)).^2 < 1;

emask = uint8(emask);
emask = emask;
MImage = MImage.*emask;% replace eye points with red pixels
